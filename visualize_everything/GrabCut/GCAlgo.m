...
gradH = im(:,2:end,:) - im(:,1:end-1,:); % horizontal gradient
gradV = im(2:end,:,:) - im(1:end-1,:,:); % vertical gradient
...
if (size(im,3) > 3)
    imageValues = [R(:) G(:) B(:) D(:)];
    bgValues = [R(bgIds)    G(bgIds)     B(bgIds)   D(bgIds)];
    fgValues = [R(fgIds)    G(fgIds)     B(fgIds)   D(fgIds)];
else
    imageValues = [R(:) G(:) B(:)];
