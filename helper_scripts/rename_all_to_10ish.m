...
for il=1:length(all_scenes)
  ...
  for jl=1:length(org_image_names)
    ...
    if(str2double(old_index) <= 659)
      new_index_s = sprintf('%06d', jl);
    else
      new_index_s = sprintf('%06d', jl+1);
    end
  ...
  try
    ...
    rename_directory('improved_depths2',meta_path,org_to_new_index_map);
    rename_directory('improved_depths3',meta_path,org_to_new_index_map);
  catch
