...
for i=1:length(all_scenes)
  ...
  for j=1:length(all_instance_names)
    ...
    %assume square grid
    grid_size = length(cur_scores_grid);
    ...
    for k=1:(grid_size-1)
      right_filter = zeros(1,k*2 +1);
      right_filter(end) = -1;    
      right_filter(k+1) = 1;
      right_results = abs(imfilter(cur_scores_grid, right_filter));

      
      left_filter = zeros(1,k*2 +1);
      left_filter(1) = -1;    
      left_filter(k+1) = 1;
      left_results = abs(imfilter(cur_scores_grid, left_filter));
      ...
      average_results = zeros(size(cur_scores_grid));

      %everything k poisitions away form boundary is good
      clean_indices = [(k+1):(grid_size-k)];
      average_results(clean_indices, clean_indices) = ...
                                      .25*right_results(clean_indices,clean_indices) + ...
                                      .25*left_results(clean_indices,clean_indices) + ...
                                      .25*forward_results(clean_indices,clean_indices) + ...
                                      .25*backward_results(clean_indices,clean_indices);
      ...
      % FORAWRD
      average_results(forward_only_rows,right_only_cols) = ...
                               .5*forward_results(forward_only_rows,right_only_cols) + ...
                               .5*right_results(forward_only_rows,right_only_cols);
      average_results(forward_only_rows,left_only_cols) = ...
                             .5*forward_results(forward_only_rows,left_only_cols) + ...
                             .5*left_results(forward_only_rows,left_only_cols);
      average_results(forward_only_rows,left_and_right_cols) = ...
                         (1/3)*forward_results(forward_only_rows,left_and_right_cols) + ...
                         (1/3)*right_results(forward_only_rows,left_and_right_cols) + ...
                         (1/3)*left_results(forward_only_rows,left_and_right_cols);
      average_results(forward_only_rows,no_left_or_right) = ...
                                 forward_results(forward_only_rows,no_left_or_right);


      %BACKWARD
      average_results(backward_only_rows,right_only_cols) = ...
                             .5*backward_results(backward_only_rows,right_only_cols) + ...
                             .5*right_results(backward_only_rows,right_only_cols);
      average_results(backward_only_rows,left_only_cols) = ...
                             .5*backward_results(backward_only_rows,left_only_cols) + ...
                             .5*left_results(backward_only_rows,left_only_cols);
      average_results(backward_only_rows,left_and_right_cols) = ...
                         (1/3)*backward_results(backward_only_rows,left_and_right_cols) + ...
                         (1/3)*right_results(backward_only_rows,left_and_right_cols) + ...
                         (1/3)*left_results(backward_only_rows,left_and_right_cols);
      average_results(backward_only_rows,no_left_or_right) = ...
                                 backward_results(backward_only_rows,no_left_or_right);


      %FORAWRD AND BACk
      average_results(forward_and_backward_rows,right_only_cols) = ...
                       (1/3)*backward_results(forward_and_backward_rows,right_only_cols) + ...
                       (1/3)*forward_results(forward_and_backward_rows,right_only_cols);
                       (1/3)*right_results(forward_and_backward_rows,right_only_cols);
      average_results(forward_and_backward_rows,left_only_cols) = ...
                  (1/3)*backward_results(forward_and_backward_rows, left_only_cols) + ...
                  (1/3)*forward_results(forward_and_backward_rows,left_only_cols);
                  (1/3)*left_results(forward_and_backward_rows,left_only_cols);
      average_results(forward_and_backward_rows,left_and_right_cols) = ...
                         .25*backward_results(forward_and_backward_rows,left_and_right_cols) + ...
                         .25*forward_results(forward_and_backward_rows,left_and_right_cols) + ...
                         .25*right_results(forward_and_backward_rows,left_and_right_cols) + ...
                         .25*left_results(forward_and_backward_rows,left_and_right_cols);
      average_results(forward_and_backward_rows,no_left_or_right) = ...
                         .5*backward_results(forward_and_backward_rows,no_left_or_right) + ...
                         .5*forward_results(forward_and_backward_rows,no_left_or_right);


      %NO FORWARD OR BACK
      average_results(no_forward_or_back,right_only_cols) = ... 
                           right_results(no_forward_or_back,right_only_cols);
      average_results(no_forward_or_back,left_only_cols) =  ...
                            left_results(no_forward_or_back,left_only_cols);
      average_results(no_forward_or_back,left_and_right_cols) =  ...
                            .5*left_results(no_forward_or_back,left_and_right_cols) + ...
                            .5*right_results(no_forward_or_back,left_and_right_cols);
      average_results(no_forward_or_back,no_left_or_right) = no_data(no_forward_or_back,no_left_or_right);
  ...
  legend(all_instance_names(7:end));
