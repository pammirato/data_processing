...
image_names = {image_names.name};
 ...
 for i=1:63%length(image_names) 
    ...
    try
        anns = load(fullfile(scene_path,'labeling','chair_boxes_per_image',rec_name));
    catch
        anns_exist = 0;
        anns = struct();
    end
...
%     end
    ...
    if(save_changes)
        annotations = anns;
        save(fullfile(scene_path,'labeling','chair_boxes_per_image',rec_name),'-struct','annotations');
    end
