...
for i=1:num_scenes
    ...
    for j=1:length(structs)
        ...
        forward_struct = structs_map(cur_struct.translate_forward);      
        two_forward_name = forward_struct.translate_forward;
        
        if(two_forward_name == -1)
            continue;
        end
        ...
        two_forward_name = strcat(forward_struct.translate_forward(1:10),'.mat');
        two_forward_rec= load(fullfile(scene_path,RECOGNITION_DIR,recognition_system_name,two_forward_name));
        two_forward_rec = two_forward_rec.dets;
        
            
        if(strcmp(category_name, 'all'))
            cur_rec = cell2mat(struct2cell(cur_rec));
            forward_rec = cell2mat(struct2cell(forward_rec));
            two_forward_rec = cell2mat(struct2cell(two_forward_rec));
            

        else
            cur_rec = cur_rec.(category_name);
            forward_rec = forward_rec.(category_name);
            two_forward_rec = two_forward_rec.(category_name);
        ...
        two_forward_rec= two_forward_rec(two_forward_rec(:,5)>score_threshold,:);

        cur_num= size(cur_rec,1);
        forward_num= size(forward_rec,1);
        two_forward_num= size(two_forward_rec,1);
        ...
        num_boxes_diff_2{end+1} = abs(cur_num - two_forward_num);
    ...
    num_boxes_diff_2 = cell2mat(num_boxes_diff_2);
